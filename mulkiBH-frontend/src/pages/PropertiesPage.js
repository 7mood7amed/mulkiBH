        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '16px', marginBottom: '36px' }}>
          <div>
            <h1 className="headline-xl" style={{ color: 'var(--primary)', fontSize: 'clamp(28px,4vw,40px)', marginBottom: '6px' }}>
              {ar ? 'اكتشف حياة استثنائية' : 'Discover Exclusive Living'}
            </h1>
            {!loading && (
          ...
          <div style={{ flex: 1 }}>
            {!loading && properties.length > 0 && (
              <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '8px', marginBottom: '20px' }}>
                <span className="label-md" style={{ color: 'var(--outline)', fontSize: '12px' }}>{ar ? 'ترتيب حسب' : 'Sort by'}</span>
                <select value={sortBy} onChange={e => setSortBy(e.target.value)} style={{
                  ...
                  <option value="newest">{ar ? 'الأحدث' : 'Newest First'}</option>
