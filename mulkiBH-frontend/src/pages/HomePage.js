...
const HomePage = () => {
  ...
  const [filters, setFilters] = useState({
    listing_type: 'rent', search: '', category: '', governorate: '', price_max: '', bedrooms: '',
  });
  ...
  const selectStyle = {
    ...
    outline: 'none', width: '100%', color: '#4a5568'
  };
  ...
  return (
    <div style={{ direction: isRTL ? 'rtl' : 'ltr' }}>
      ...
      <div style={{
        ...
        color: 'white', padding: '60px 20px 50px', textAlign: 'center',
        ...
        <h1 className="fade-in-up" style={{ fontSize: 'clamp(22px, 5vw, 38px)', marginBottom: '12px', fontWeight: 'bold', lineHeight: '1.3' }}>
          {t('heroTitle')}
        </h1>
        ...
        <div className="fade-in-up delay-2" style={{
          ...
          <div style={{
            ...
            <input
              type="number" value={filters.price_max}
          ...
          <button onClick={handleSearch} className="btn-gold" style={{
      ...
      {/* Features Section */}
      <div style={{ padding: '56px 20px', background: '#f8f9fa' }}>
        <h2 style={{ textAlign: 'center', color: '#1a3c5e', marginBottom: '8px', fontSize: 'clamp(20px, 3vw, 28px)' }}>
          {isRTL ? 'لماذا ملكي؟' : 'Why MulkiBH?'}
        </h2>
        <p style={{ textAlign: 'center', color: '#718096', marginBottom: '36px', fontSize: '15px' }}>
          {isRTL ? 'المنصة العقارية الأولى في البحرين' : "Bahrain's premier real estate platform"}
        </p>
        <div style={{
      ...
      <div style={{ background: '#1a3c5e', padding: '48px 20px', color: 'white', textAlign: 'center' }}>
        <div style={{
          ...
          {[
            { num: '4', suffix: '', label: isRTL ? 'محافظات' : 'Governorates' },
            { num: '40', suffix: '+', label: isRTL ? 'منطقة' : 'Areas' },
            { num: '3', suffix: '', label: isRTL ? 'فئات عقارية' : 'Property Types' },
            { num: '100', suffix: '%', label: isRTL ? 'بحريني' : 'Bahrain Focused' },
          ].map((s, i) => (
            <div key={i}>
